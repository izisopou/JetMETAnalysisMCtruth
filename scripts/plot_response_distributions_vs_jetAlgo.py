...
def main():
    usage = 'Example: python3 plot_response_distributions_vs_jetAlgo.py --era Winter25 --version V1 --JEC 0 --JetPtMin 30 --JetPtMax 35' 
    ...
    for eta_pair in eta_pairs:
        ...
        frame = c1.DrawFrame(0, 0.00005, 3.5, 1)
        ...
        legend = rt.TLegend(0.67,0.37,0.92,0.72,"")
        ...
        for jet_algo, jet_algo_name, color in zip(jet_algos, jet_algos_names, colors):
            ...
            root_filename = '/eos/cms/store/group/phys_jetmet/ilias/JEC_NewMethods_Run3/Responses/' + ('' if args.jec else 'Un') + 'CorrectedResponses_' + jet_algo_name.replace(" ", "") + '_' + args.era + ( ('_' + args.version) if args.jec else '') + '.root' 
        
            print('Processing file: ' + root_filename)
        
            root_file = rt.TFile(root_filename, 'READ')
                
            hist_filename = (jet_algo + '/RelRspVsRefPt_JetEta' + eta_min + 'to' + eta_max)
            ...
            hist = hist2D.ProjectionY('h_' + jet_algo, ptbin, ptbin, 'e')
            ...
            leg_entry = '#splitline{%s}{#splitline{#scale[0.85]{Median = %.3f}}{#scale[0.85]{Mean = %.3f}}}' % (jet_algo_name, median, hist.GetMean())
        ...
        lumi = rt.TPaveText(0.732, 0.91, 0.852, 0.95, "NDC")
        ...
        lvert = rt.TLine(1, 0.00005, 1, 1)
        lvert.SetLineWidth(2)
        lvert.SetLineStyle(rt.kDashed)
        lvert.Draw()


        output_png = '../Plots/Responses/' + ('' if args.jec else 'Un') + 'CorrectedResponses_' + args.era + '_AK4PUPPI_vs_AK8PUPPI_vs_AK4CHS_JetAbsEta' + eta_min + 'to' + eta_max + '_JetPt' + str(int(args.jet_pt_min)) + 'to' + str(int(args.jet_pt_max)) + '.png'
